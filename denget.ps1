...
function getrclone() {
   ...
   if (Get-Command rclone -ErrorAction SilentlyContinue) # check if rclone is installed globally
   {
      return "rclone"
   }
   else
...
if($cmd -eq "upgrade")
{
   ...
   if ($name -eq "denget")
   {
      ...
      $tag = (Invoke-WebRequest "https://api.github.com/repos/mutr0p/congenial-fishstick/releases" | ConvertFrom-Json)[0].tag_name
      ...
      $download = "https://github.com/mutr0p/congenial-fishstick/releases/download/$tag/upgrade.ps1"
      Invoke-WebRequest $download -OutFile "$dgpath\temp\upgrade.ps1"
      $global:ProgressPreference = $oldProgressPreference

      & $dgpath\temp\upgrade.ps1 -dgpath $dgpath -curver $dgver
...
if($cmd -eq "status")
{
   ...
   else
   {
      ...
      Write-Host "Checking for apps updates..."
      foreach ($item in $list)
      {
         $name = $item.name
         $bucket = $item.bucket
...
if($cmd -eq "cloud")
{
   ...
   if ($name -eq "support")
   {
      ...
      Write-Host "When installed via denget, it will automatically be used as a default rclone for cloud support."
      Write-Host ""
      Write-Host "You can also install rclone globally using any method available. It's only required to be available via 'rclone' command from your terminal."
...
if($cmd -eq "features")
{
   Write-Host "denget supports public, private and bittorrent sources for apps."

   Write-Host "public" -f gre -n; Write-Host " - always enabled."

   if ($null -eq $(getrclone))
   {
      Write-Host "private" -f r -n; Write-Host " - disabled, refer to '" -n; Write-Host "denget cloud support" -f m -n; Write-Host "'."
   }
   elseif (!($(find rclone)))
   {
      Write-Host "private" -f y -n; Write-Host " - enabled, but local rclone is recommended. See '" -n; Write-Host "denget cloud support" -f m -n; Write-Host "'."
   }
   else
   {
      Write-Host "private" -f gre -n; Write-Host " - enabled."
   }
...
if($cmd -eq "config")
{
   ...
   Write-Host "Choose a file to edit: " -NoNewline
   $choice = Read-Host
...
if($cmd -eq "about")
{
   ...
   Write-Host "denget is a package manager with support for downloading portable applications and regular installers from public`nsources and private sources via rclone."
   ...
   Write-Host "P.S. Special thanks to Masha Machula who made this possible <3"
}